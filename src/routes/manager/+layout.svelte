<script lang="ts">
	...
	import { Calendar, Users, BarChart3, LogOut, Menu, X } from 'lucide-svelte';
	...
	<div class="flex">
		...
		<aside class="hidden lg:flex lg:flex-col lg:w-64 bg-white border-r border-gray-200 min-h-screen">
			<div class="p-6 border-b border-gray-200">
				...
				<p class="text-sm text-gray-600 mt-1">Manager Portal</p>
			</div>

			<nav class="flex-1 p-4 space-y-2">
				<a
					href="/manager/dashboard"
					class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors {currentPath === '/manager/dashboard'
				...
				<a
					href="/manager/jobs"
					class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors {currentPath.startsWith('/manager/jobs')
						? 'bg-primary-50 text-primary-700'
						: 'text-gray-700 hover:bg-gray-100'}"
				>
				...
				<a
					href="/manager/staff"
					class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors {currentPath.startsWith('/manager/staff')
						? 'bg-primary-50 text-primary-700'
						: 'text-gray-700 hover:bg-gray-100'}"
				>
					<Users class="w-5 h-5" />
			...
			<div class="p-4 border-t border-gray-200">
				<div class="px-4 py-3 mb-2">
					<p class="text-sm font-medium text-gray-900">{profile?.name || 'Manager'}</p>
